...
class Hub(tk.Tk):
    def __init__(self):
        ...
        self.title('InSightec Complaint Service Hub'); self.geometry('1280x840'); self.minsize(1120,720); self.configure(bg='#F5F8FC')
        self.login(); self.language_var=tk.StringVar(value=self.current_language()); self.output_var=tk.StringVar(value=self.data.settings.get('default_output_mode','Copy Template')); self.build()
    ...
    def set_output_mode(self, event=None):
        ...
        self.data.settings['default_output_mode'] = mode
        self.data.save_settings()
    ...
    def build(self):
        ...
        tk.Label(self.sidebar,text='Bringing therapy into focus',bg='#07355F',fg='#D7ECFF',font=('Segoe UI',10,'bold italic')).pack(anchor='w',padx=28,pady=(0,22))
        box=tk.Frame(self.sidebar,bg='#0B4A82',highlightbackground='#00A4EF',highlightthickness=1); box.pack(fill='x',padx=18,pady=12)
        tk.Label(box,text='Complaint Service Hub\nVer 0.4',justify='left',bg='#0B4A82',fg='white',font=('Segoe UI',12,'bold')).pack(anchor='w',padx=14,pady=12)
        ...
        tk.Label(self.sidebar,text='\nCompany: '+self.session['company']['display_name']+'\nUser: '+self.session['user']['name']+'\nOutput: '+self.output_var.get(),bg='#07355F',fg='#D7ECFF',justify='left',font=('Segoe UI',9)).pack(side='bottom',anchor='w',padx=18,pady=18)
        self.main=tk.Frame(self,bg='#F5F8FC'); self.main.pack(side='left',fill='both',expand=True)
        self.home()
    def header(self):
        h=tk.Frame(self.main,bg='#F5F8FC'); h.pack(fill='x',padx=30,pady=(22,8))
        title=tk.Frame(h,bg='#F5F8FC'); title.pack(side='left')
        tk.Label(title,text='InSightec',fg='#005DAA',bg='#F5F8FC',font=('Segoe UI',30,'bold')).pack(side='left')
        tk.Label(title,text=' Complaint Service Hub',fg='#1B1F2A',bg='#F5F8FC',font=('Segoe UI',22,'bold')).pack(side='left',padx=10)
        ctrl=tk.Frame(h,bg='#F5F8FC'); ctrl.pack(side='right')
        if self.session['company']['company_id']=='insightec' and self.session['user'].get('admin_switch_allowed'):
            ...
            tk.Button(ctrl,text=txt,bg='#FFD24A' if self.admin_mode else 'white',fg='#07355F',command=self.toggle_admin,relief='solid',bd=1).pack(side='right',padx=5,ipadx=10,ipady=7)
        ttk.Label(ctrl,text='Output').pack(side='left',padx=(10,4))
        out_cb=ttk.Combobox(ctrl,textvariable=self.output_var,state='readonly',width=14,values=['Outlook','Copy Template'])
        out_cb.pack(side='left'); out_cb.bind('<<ComboboxSelected>>', self.set_output_mode)
        ttk.Label(ctrl,text='Language').pack(side='left',padx=(14,4))
        lang_cb=ttk.Combobox(ctrl,textvariable=self.language_var,state='readonly',width=10,values=list(LANGUAGES.keys()))
        lang_cb.pack(side='left'); lang_cb.bind('<<ComboboxSelected>>', self.set_language)
        tk.Label(ctrl,text=self.session['user']['name'],fg='#07355F',bg='white',font=('Segoe UI',10,'bold'),relief='solid',bd=1,padx=14,pady=8).pack(side='left',padx=(14,0))
        sub=tk.Frame(self.main,bg='#F5F8FC'); sub.pack(fill='x',padx=34)
        tk.Label(sub,text='Company: '+self.session['company']['display_name']+'   |   Language: '+LANGUAGES.get(self.language_var.get(), self.language_var.get())+'   |   Output: '+self.output_var.get(),bg='#F5F8FC',fg='#223',font=('Segoe UI',11)).pack(side='left')
        tk.Frame(self.main,bg='#B6D4EE',height=1).pack(fill='x',padx=30,pady=14)
    def home(self):
        for w in self.main.winfo_children(): w.destroy()
        self.header()
        wrap=tk.Frame(self.main,bg='#F5F8FC'); wrap.pack(fill='both',expand=True,padx=28,pady=4)
        ...
        tk.Label(wrap,text='Launch Pad',bg='#F5F8FC',fg='#07355F',font=('Segoe UI',16,'bold')).grid(row=0,column=0,columnspan=3,sticky='w',padx=12,pady=(0,4))
        row=1
        for i,key in enumerate(user_feats): self.card(wrap,key,row+i//3,i%3)
        row += (len(user_feats)+2)//3
        if admin_feats:
            sep=tk.Label(wrap,text='Admin Only',bg='#F5F8FC',fg='#6A35AD',font=('Segoe UI',13,'bold'))
        ...
        foot=tk.Frame(self.main,bg='#07355F',height=48); foot.pack(fill='x',side='bottom')
        status='  ✓ Ready     Version: 0.4     Master: Loaded     Language: '+LANGUAGES.get(self.language_var.get(), self.language_var.get())+'     Output: '+self.output_var.get()
        tk.Label(foot,text=status,bg='#07355F',fg='white',font=('Segoe UI',10)).pack(side='left',pady=13)
    def card(self,parent,key,r,c,admin=False):
        title,desc,color=FEATURES[key]
        f=tk.Frame(parent,bg='white',highlightbackground='#D5E6F5',highlightthickness=1); f.grid(row=r,column=c,sticky='nsew',padx=12,pady=12)
        parent.columnconfigure(c,weight=1); parent.rowconfigure(r,weight=1)
        stripe=tk.Frame(f,bg=color,height=5); stripe.pack(fill='x')
        tk.Label(f,text=title,bg='white',fg=color,font=('Segoe UI',16,'bold')).pack(anchor='w',padx=24,pady=(20,8))
        tk.Label(f,text=desc,bg='white',fg='#222',wraplength=280,justify='left',font=('Segoe UI',10)).pack(anchor='w',padx=24,pady=4)
        badge='Available to all users' if key=='settings' else ('Admin only' if admin else 'User tool')
        tk.Label(f,text=badge,bg='#EEF6FC',fg='#07355F',font=('Segoe UI',9,'bold'),padx=8,pady=3).pack(anchor='w',padx=24,pady=(8,0))
